...
// `(add X, Y) ne X` is equivalent to `Y ne 0`: the shared `X`
...
// CHECK:      %[[R:.*]] = "llvm.icmp"(%[[Y]], %[[Z]]) <{"predicate" = 1 : i64}> : (i64, i64) -> i1
// CHECK:      "func.return"(%[[R]]) : (i1) -> ()

// Unshared operand: the comparison is left intact.
// CHECK:      ^{{.*}}(%[[NX:.*]] : i64, %[[NY:.*]] : i64, %[[NW:.*]] : i64):
// CHECK:      %[[NOP:.*]] = "llvm.add"(%[[NX]], %[[NY]]) : (i64, i64) -> i64
// CHECK:      %[[NR:.*]] = "llvm.icmp"(%[[NOP]], %[[NW]]) <{"predicate" = 1 : i64}> : (i64, i64) -> i1
