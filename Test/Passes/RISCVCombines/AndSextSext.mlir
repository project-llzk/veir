...
// A sign extension distributes over `and`: sign-extending the inputs then anding is the same as anding then sign-extending, because sign extension replicates the top bit uniformly. The two casts are hoisted through
// the bitwise op: `(sext X) and (sext Y) -> sext (X and Y)`, doing the
