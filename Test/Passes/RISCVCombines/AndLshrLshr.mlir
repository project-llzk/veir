...
// Distributing `and` through a shared logical right shift: `(X >> Z) & (Y >> Z)` equals `(X & Y) >> Z`. Only fires when both logical right shifts share the same
// second operand `Z`.
...
// The `and` now feeds a single `lshr`.
