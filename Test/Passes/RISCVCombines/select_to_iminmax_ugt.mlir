...
// `(X ugt Y) ? X : Y` folds to `llvm.intr.umax X Y`. The compared operands
...
// The select+icmp collapse into a single umax intrinsic.
// CHECK:      ^{{.*}}(%[[X:.*]] : i64, %[[Y:.*]] : i64):
// CHECK:      %[[M:.*]] = "llvm.intr.umax"(%[[X]], %[[Y]]) : (i64, i64) -> i64
