...
// Distributing `or` through a shared arithmetic right shift: `(X >> Z) | (Y >> Z)` equals `(X | Y) >> Z` (the replicated sign bits agree bit-by-bit). Only fires when both arithmetic right shifts share the same
// second operand `Z`.
...
// The `or` now feeds a single `ashr`.
