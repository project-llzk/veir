...
// Distributing `xor` through a shared arithmetic right shift: `(X >> Z) ^ (Y >> Z)` equals `(X ^ Y) >> Z` (the replicated sign bits cancel bit-by-bit). Only fires when both arithmetic right shifts share the same
// second operand `Z`.
...
// The `xor` now feeds a single `ashr`.
