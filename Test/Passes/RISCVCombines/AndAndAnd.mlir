...
// Distributing `and` through a shared masking `and`: `(X & Z) & (Y & Z)` equals `(X & Y) & Z`. Only fires when both inner `and`s share the same
// masking operand `Z`.
...
// The `and` now feeds a single masking `and`: (X and Y) & Z.
...
// CHECK:      %[[NR:.*]] = "llvm.and"(%[[NAX]], %[[NAY]]) : (i64, i64) -> i64
