...
// `(X sgt Y) ? X : Y` folds to `llvm.intr.smax X Y`. The compared operands
...
// The select+icmp collapse into a single smax intrinsic.
// CHECK:      ^{{.*}}(%[[X:.*]] : i64, %[[Y:.*]] : i64):
// CHECK:      %[[M:.*]] = "llvm.intr.smax"(%[[X]], %[[Y]]) : (i64, i64) -> i64
