...
// A zero extension distributes over `xor`: zero-extending the inputs then xoring is the same as xoring then zero-extending, because the added high bits are all zero. The two casts are hoisted through
// the bitwise op: `(zext X) xor (zext Y) -> zext (X xor Y)`, doing the
